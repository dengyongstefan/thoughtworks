...
class  klass{
    ...
    assignLeader(student,teacher){
        ...
        let result="";
        for(let banji of teacher.klasses){
            if(banji==student.klass.number){
                result=`I am ${teacher.name}. I know ${student.name} has become Leader of Class ${student.klass.number} .`;
            }
        }
        console.log(result);
    }
    appendMember(student,techer,nmber){
        student.whethercontain=true;
        student.klass.number=nmber;
            let result="";
            for(let banji of techer.klasses){
                if(banji==student.klass.number){
                    result=`I am ${techer.name}. I know ${student.name} has joined Class ${student.klass.number}.`;
                }
            }
            console.log(result);
    }
...
klass1.appendMember(daming,erming,3);
klass1.assignLeader(daming,erming);
